...
import { getAvatarWalrusId } from '../lib/profileStorage';
...
export function ProfilePage() {
  ...
  const handle = appState.username ? `@${appState.username}` : appState.address;
  const avatarSeed = appState.address ?? 'guest';
    ...
    <div className="max-w-lg mx-auto md:max-w-xl px-4 md:px-0">
      <div className="flex flex-col items-center text-center mb-8">
        <Avatar
          ...
          walrusMediaId={appState.address ? getAvatarWalrusId(appState.address) : undefined}
          alt={displayName}
          size="xl"
          className="mb-4"
        />
        ...
        <p className="text-2xl font-bold mt-4">${appState.balanceUsdc.toFixed(2)}</p>
      ...
      <div className="flex flex-col gap-2">
        <ProfileLink
          ...
          icon={Wallet}
          label="Wallet"
          detail={`$${appState.balanceUsdc.toFixed(2)}`}
        />
      ...
      {appState.subscribedCreators.length > 0 && (
        <div className="mt-8">
          <h2 className="text-sm font-semibold text-muted uppercase tracking-wide mb-3">
            Subscriptions
          </h2>
          <p className="text-sm text-ink">
            {appState.subscribedCreators.length} active subscription
            {appState.subscribedCreators.length > 1 ? 's' : ''}
          </p>
        </div>
      )}
    </div>
